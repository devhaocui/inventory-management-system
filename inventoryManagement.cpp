...
void invManage::addItem (std::string iName, std::string iCategory, int iQuantity)
{
  ...
  if (!newItemAdded || vec.empty())
  {
    ...
    file << temp.itemName << ',' << temp.itemCategory << ',' << temp.itemQuantity << ',';
    file.close();
  }
}
...
void invManage::withdrawItem(std::string iName, int withdrawAmount)
{
  ...
  for (size_t i{0}; i < vec.size(); i++)
  {
    ...
    if (iName == vec[i].itemName)
    {
      ...
      {
        std::cout << "Withdrew \x1b[31m[Failed]\x1b[0m. You tried to withdraw (" << withdrawAmount << ") " << iName << " from the inventory.\n";
        std::cout << "There's (" << vec[i].itemQuantity << ") " << iName << " left in the inventory.";
      }
      else
      {
        std::cout << "Withdrawing \x1b[32m(" << withdrawAmount << ")\x1b[0m " << iName << " from the inventory\n";
        std::cout << "There's (" << vec[i].itemQuantity << ") " << iName << " left in the inventory.";
  ...
  for (size_t i{0}; i < vec.size(); i ++)
  {
    if (iName == vec[i].itemName)
    {
      ...
      std::cout << "\n\x1b[32m=====Item Found=====\x1b[0m\n\x1b[33mItemName |" 
                ...
                << iName << std::setw(width+3) << vec[i].itemCategory << std::setw(width+2)
                << vec[i].itemQuantity << "\x1b[0m\n";
    }
  }
  if (!itemFound)
